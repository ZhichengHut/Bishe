...
void saveTrainData(string img_file, string csv_file, string out_fold, float thresh){
...
void saveTrainData(string img_file, string out_fold, float thresh){
	Mat img = imread(img_file,1);
	Mat blue_ratio = preProcess(img, thresh);
...
void getTrainingSet(string train_fold, string out_fold, float thresh){
	...
	for(int i=0; i<tif_set.size(); i++){
		if(j<csv_set.size() && tif_set[i].substr(train_fold.length(),5) == csv_set[j].substr(train_fold.length(),5)){
			cout << tif_set[i] << endl;
			//cout << csv_set[j] << endl;
			saveTrainData(tif_set[i], csv_set[j++], out_fold, thresh);
		}
		else{
			cout << tif_set[i] << endl;
			saveTrainData(tif_set[i], out_fold, thresh);
		}
	}
}

void extractData(string train_fold, string out_fold, float train_thresh, bool get_train){
	if(get_train){
		getTrainingSet(train_fold, out_fold, train_thresh);
